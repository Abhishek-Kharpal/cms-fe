...
const Home = () => {
  ...
  const handleMount = async () => {
    await makeRequest(GET_ALL_COLLECTIONS, {}, navigate, BACKEND_URL).then((res) => {
      setCollections(res);
    });

    await makeRequest(GET_ALL_FIELDS, {}, navigate, BACKEND_URL).then((res) => {
      setFields(res);
    });
  };
  ...
  const onEdit = async (data, event) => {
    ...
    if (res) {
      ...
      setFields(newFields);
      const selectedCollectionsData = collections.filter(
        (item) => item.name === selectedCollection,
      );
      setSelectedFields(
        newFields.filter((item) => item.collectionId === selectedCollectionsData[0].id),
      );
    }
  ...
  const handleSelectedFields = (collectionName) => {
    ...
    const selectedFieldsData = fields.filter(
      (item) => item.collectionId === selectedCollectionsData[0].id,
    );
        ...
        <div className='collection-holder basic-padding'>
          ...
          <ul>
            {collections.map((item) => (
              <li key={item.id} onClick={() => handleCollection(item.name)}>
                {item.name}
              </li>
            ))}
        ...
        <div className='content-holder'>
              ...
              <button
                ...
                style={
                  selectedCollection === item.name
                    ? { backgroundColor: 'rgb(79, 33, 198)', color: '#f5f5f5' }
                    : {}
                }
              >
                {item.name}
          ...
          {selectedCollection ? (
            <div className='selected-content basic-padding'>
              ...
              <button className='add-content-type' onClick={handleAddField}>
                Add another field
              </button>
